...
void init(int n){
    rrep(i,n){
        ...
        //rrep(j,n) a[i][j] = b[i][j] = 0; 
    }
    rrep(i,n*n) mv[i] = 0; 
   // rrep(i,n*n) ax[i]=ay[i]=bx[i]=by[i]=0; 
}
...
void solve(){
    ...
    bool RRR = false; 
    rrep(i,n) if(!fixed_c[i]) RRR = true; 
    auto rotate_r = [&](int r, int t){
        // v.begin()+t 가 앞을 가리키게 됨 
        if(t<0) t+=n; 
        if(rr.find(t)!=rr.end()&&RRR){
            return false; 
        }
        rr.insert(t); 
    ...
    bool CCC = false; 
    rrep(i,n) if(!fixed_r[i]) CCC = true; 
    set<int> cc; 
    auto rotate_c = [&](int c, int t){
        // v.begin()+t 가 앞을 가리키게 됨 
        if(t<0) t+=n; 
        if(cc.find(t)!=cc.end()&&CCC){
            return false; 
        }
        cc.insert(t); 
    ...
    while(cnt){
        int p=0; 
        bool qqqq = false; 
        rrep(i,n) if(!fixed_r[i]){
            int ff = -1000;  
            bool kiki = true; 
            rrep(j,n) if(fixed_c[j]){
                int go = b[i][j]; 
                if(ax[go]==i){
                    int uu = ay[go]-j; if(uu<0) uu+=n;
                    if(ff==-1000) ff=uu; 
                    else if(ff!=uu) {
                        kiki= false; break; 
                    } 
