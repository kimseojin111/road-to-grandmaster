...
void solve(){
    ...
    #define CAST(X) std::bitset<2 * sizeof(int) * 8>  // 2 integers = 2 * sizeof(int) * 8 bits
    #define ACCESS(X) *((std::bitset<2 * sizeof(int) * 8>*)&X)
    ...
    auto chk = [&](pair<int,int> x, int dnow) {
        dbg(x, dnow);
        if (vis[CAST(x)]) return;  // Use CAST(x) for storing in the map
    ...
    while (!q.empty()) {
        ...
        if (vis[CAST({a, b})]) continue;  // Use CAST({a, b}) for accessing the map
        dis[CAST({a, b})] = d;  // Use CAST({a, b}) for storing the distance
        vis[CAST({a, b})] = 1;  // Use CAST({a, b}) for marking as visited
    ...
    if (!vis[CAST({c, d})]) {
        ...
        cout << dis[CAST({c, d})] << "\n";  // Use CAST({c, d}) for accessing the distance
    }
